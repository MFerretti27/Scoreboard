...
- Email notifications (diagnostic alerts) can be configured via environment variables (auto-loaded from `.env` if present) or fall back to defaults in [helper_functions/email.py](../helper_functions/email.py).
- Variables: `EMAIL_ENABLED`, `SMTP_SERVER`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASSWORD`, `TO_EMAILS` (comma-separated).
- A template is provided at [.env.example](../.env.example). Create your own `.env` (not committed) and set values as needed.
- Do not commit real credentials. Keep your `.env` local.
- Logs may be attached to error emails for troubleshooting. Avoid sharing logs publicly if they could contain sensitive information.
- Wi‑Fi SSID and password can be entered in the UI to connect on devices like Raspberry Pi. Treat these as secrets and avoid capturing screenshots or logs that include them.

### Email Setup
Preferred: Create a `.env` file (based on `.env.example`) at the repo root:
...
Alternatively, you can set these variables in your OS environment. If not provided, the app uses safe defaults from `helper_functions/email.py`.

Tips:
- Use an app password (recommended). For Gmail, enable 2‑factor auth and create an App Password.
- Keep `.env` out of version control.
- To disable emails in development, set `EMAIL_ENABLED=false` in your `.env`.
...
## Dev Troubleshooting
- Pip/venv issues (e.g., IndentationError in pip): recreate the virtual environment and reinstall requirements.
...
- Disable diagnostic emails during development to avoid send attempts:
    - Set `EMAIL_ENABLED=false` in your `.env` (see `.env.example`).

- Environment variables: the app auto-loads `.env` (via python-dotenv) if present. Prefer `.env` over editing code for secrets.
