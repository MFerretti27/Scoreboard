...
from helper_functions.api_utils.validators import validate_nba_boxscore
...
def get_nba_player_stats(team_name: str) -> tuple[str, str]:
    ...
    """
    with log_context_scope(team=team_name, league="NBA", endpoint="nba_player_stats"):
        ...
        try:
            validate_nba_boxscore(box_score, team_name)
        except DataValidationError as e:
            msg = f"Invalid NBA boxscore data: {e!s}"
            raise APIError(
                msg,
...
def get_nhl_player_stats(team_name: str) -> tuple[str, str]:
    ...
    :return: Tuple containing home and away player statistics strings
    """
    with log_context_scope(team=team_name, league="NHL", endpoint="nhl_player_stats"):
...
def get_mlb_player_stats(team_name: str) -> tuple[str, str]:
    ...
    :return: Tuple containing home and away player statistics strings
    """
    with log_context_scope(team=team_name, league="MLB", endpoint="mlb_player_stats"):
        home_player_stats = ""
        away_player_stats = ""
...
def get_nfl_player_stats(team_name: str) -> tuple[str, str]:
    ...
    :return: Tuple containing home and away player statistics strings
    """
    with log_context_scope(team=team_name, league="NFL", endpoint="nfl_player_stats"):
        ...
        for res in [nfl.json()]:
            for event in res["events"]:
                ...
                rush_name = " ".join(
                    competition.get("leaders", {})[1]["leaders"][0]["athlete"]["shortName"].split()[1:],
                )
